function ADMM_graph_result(hist, k, LC_vec)
figure(19)
...
for i=1:k-1
    es(i) = hist(i).Es(4,5);
    eb(i) = hist(i).Eb(4,5);
    eaverage(i) = hist(i).Eaverage(4,5);
    zglob(i) = hist(i).zglob(4,5);
    sellprice(i) = hist(i).Lambda_s(4,5);
    buyprice(i) = hist(i).Lambda_b(4,5);
end
subplot(2,2,1)
...
hold off
grid on
leg1 = legend([{"$e^s_{34}$"}, {"$e^b_{34}$"}, {"$\bar{e}_{34}$"}, {"$z_{34}$"}]);
...
hold off
grid on
leg2 = legend([{"$\lambda^s_{34}$"}, {"$\lambda^b_{34}$"}]);
...
for i=1:k-1
    es(i) = hist(i).Es(6,2);
    eb(i) = hist(i).Eb(6,2);
    eaverage(i) = hist(i).Eaverage(6,2);
    zglob(i) = hist(i).zglob(6,2);
    sellprice(i) = hist(i).Lambda_s(6,2);
    buyprice(i) = hist(i).Lambda_b(6,2);
end
subplot(2,2,3)
pp1 = plot(1:(k-1),es,'LineWidth',2, 'color',[0 0.4 0.8]);
hold on
